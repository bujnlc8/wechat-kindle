...
if($num>0){
    ...
    echo "<div class='table-responsive'><table class='bordered highlight' style='table-layout:fixed;width:100%;'><tr><th style='width:6%'>序号</th><th width=20%>书名</th><th width=15%>作者</th><th style='width:45%'>链接</th>";
	if(!is_mobile()){
		echo "<th>操作</th></tr>";
	}else{
		echo "</tr>";
...
while($book=mysqli_fetch_row($result)) {
    ...
    if(!strstr($book[3],'链接')){
		if(!isWechat()){
		  if(!is_mobile()){
           echo  "<td style='font-size:0.7em;color:blue;' class='url'><a href='downloadFile.php?url=$book[3]&bookName=$book[1]' >$book[3]</a></td>";
