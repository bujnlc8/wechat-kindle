        ...
        switch ($object->Event) {
            ...
            case "CLICK":
                ...
                if($content=='haihuilingWeixinBook'){
                    ...
                    $content = "这是一个个人电子书分享的公众号，里面包含了我这几年收集的所有电子书，包括在亚马逊kindle商店自费的和逛电子书分享论坛获取的，大部分为原版,大家可以根据自己的需要免费下载。由于可能涉及到版权问题，所以恳请大家稍稍保密，如果有些内容危害到您的权益，请通知我，我会第一时间删除。另外下载的内容严禁商用，如果有任何疑问，请联系微信haihuilinghuster,谢谢大家的支持！最后祝大家阅读愉快！";
            ...
            if (null!=$bookName&&$bookName != "") {
                ...
                if ( $result = mysqli_query($con, $sql)) {
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        if($num%2==0){
                            $item_str .= sprintf($itemTpl, $row['book_name'] . "-" . $row['book_writer'], $row['book_name'], "http://haihuiwechat.sinaapp.com/res/img/book_brown.png", "http://haihuiwechat.sinaapp.com/bookDetail.php?bookurl=".$row['book_url']."&bookName=".$row['book_name']."&openid=".$openid);
                        }else{
                        $item_str .= sprintf($itemTpl, $row['book_name'] . "-" . $row['book_writer'], $row['book_name'], "http://haihuiwechat.sinaapp.com/res/img/book_green.png", "http://haihuiwechat.sinaapp.com/bookDetail.php?bookurl=".$row['book_url']."&bookName=".$row['book_name']."&openid=".$openid);  
                       }
                        $num++;
                        if($num==8){
                            break;
